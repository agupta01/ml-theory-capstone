...
def classify(type: str, train_data, test_data, return_metrics: bool = True, **kwargs):
    ...
    **kwargs
    ...
    yhat = np.sign(
        (kernel(x_train, x_train) @ alpha_hat) + 1e-12
    ...
    recall = tp / (tp + fn)
    if return_metrics:
        result = "TRAIN MSE: %.3f" % np.mean(
            (yhat - y_train) ** 2
        ) + " | Accuracy: %.3f" % np.mean(yhat == y_train)
        result += (
            "\n TEST MSE: %.3f" % np.mean((yhat_test - y_test) ** 2)
            + " | Accuracy: %.3f" % np.mean(yhat_test == y_test)
